...
//	public List<ProductsInCategoryDto> getAllProductByCategory(@Param("categoryId") String categoryId);
	...
	@Query(value = "SELECT p.id,  p.name as nameProduct,  p.price,  p.rate,  p.number_buy ,  imgp.name_image as nameImg "
			+ " FROM product as p"
			+ " INNER JOIN image imgp ON p.id=imgp.product_id"
			+ " WHERE p.category_id=:categoryId and imgp.index_image=0",
			nativeQuery = true)
