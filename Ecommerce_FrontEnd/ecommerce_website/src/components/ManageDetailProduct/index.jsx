...
import { apiUploadImage, apiAdmin, apiCategory, apiProduct } from '../../api';
...
import ModalLoadding from '../ModalLoadding';
...
const ManageDetailProduct = ({ idProduct }) => {
    ...
    const onSubmitCreateProduct = async (data) => {
        ...
        await apiAdmin.fetchUpdateProduct(productEdit).then(async (res) => {
            ...
            if (isChangeColor) {
                await apiAdmin.fetchDeleteColorProduct(idProduct).then((res) => { }).catch((err) => console.log(err));
                await apiAdmin.fetchCreateColorProduct({ listAttributes: selectedOptionColor, productId: idProduct })
                    .then((res) => { }).catch((err) => console.log(err));
            }
            if (isChangeSize) {
                await apiAdmin.fetchDeleteSizeProduct(idProduct).then((res) => { }).catch((err) => console.log(err));
                await apiAdmin.fetchCreateSizeProduct({ listAttributes: selectedOptionSize, productId: idProduct })
                    .then((res) => { }).catch((err) => console.log(err));
            }
            if (isChangeImg) {
                await apiAdmin.fetchDeleteImageProduct(idProduct).then((res) => { console.log('fetchDeleteImageProduct success') }).catch((err) => console.log(err));
                ...
                await apiAdmin.fetchCreateImageProduct({ listImages: listImageInCloud }).then((res) => {
                    handleCloseLoading();
                    handleShowModal();
    ...
    const fetchSize = async () => {
        await apiAdmin.fetchGetSize().then((res) => {
    ...
    const fetchColor = async () => {
        await apiAdmin.fetchGetColor().then((res) => {
            let tempSize = res.data.dataResponse.map((item) => {
                return { ...item, label: item.name, value: item.name }
            });
            setColor(tempSize);
    ...
    const fetchDetailProduct = () => {
        apiProduct.fetchGetDetailProduct(idProduct)
            .then((res) => {
                console.log('imageProduct', res.data.imageProduct);
