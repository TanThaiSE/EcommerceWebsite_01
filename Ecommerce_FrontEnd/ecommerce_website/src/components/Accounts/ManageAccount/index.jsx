...
const ManagerAccount = () => {
  ...
  const fetchGetUsers = (page, offset) => {
    apiUser.fetchGetUsers(page, offset).then((res) => {
  ...
  const fetchUpdateBlockedUser = (account) => {
    ...
    let id = account.id;
    apiUser.fetchUpdateBlockedUser(idCustomer)
      .then((res) => {
  ...
  return (
    <div>
      ...
      <div className='d-flex justify-content-between mt-5 mb-3'>
        <div>
          <button className='btn btn-success' onClick={() => { navigate('/create-customer') }}>Create Customer</button>
        </div>
        <div className='d-flex' >
      ...
      {lstUsers.length !== 0 ? (<ReactPaginate
        breakLabel='...'
        nextLabel='>'
